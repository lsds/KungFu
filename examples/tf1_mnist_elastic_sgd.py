...
class ElasticSGDHook(tf.train.SessionRunHook):
  def __init__(self, num_epochs, global_batch_size):
    self._num_epochs = num_epochs
    self._global_batch_size = global_batch_size
    self._num_steps = self._num_epochs // global_batch_size
    self._last_step = None

  def begin(self):
    self._global_step_tensor = tf.train.get_or_create_global_step()
    if self._global_step_tensor is None:
      raise RuntimeError("Global step should be created to use ElasticSGDHook.")
  ...
  def after_run(self, run_context, run_values):
    ...
    print("ElasticSGDHook: global step = %s" % str(global_step))
...
def main(_):
    global_batch_size = FLAGS.batch_size * current_cluster_size()
    hooks = [
        BroadcastGlobalVariablesHook(),
        tf.train.LoggingTensorHook(['train_accuracy', 'train_loss'],
                                   every_n_iter=10),
        ElasticSGDHook(FLAGS.num_epochs, global_batch_size),
    ]
