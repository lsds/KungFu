...
public:
    ...
    tensor_meta(std::string name, int32_t size) {
        this->name = name;
        this->size = size;
    }
    ...
    {
        o << "TensorMeta{name=" << t.name << ", size=" << t.size << "}" << std::endl;
        return o;
    }
...
public:
    ...
    friend std::ostream& operator<<(std::ostream& os, const partition& p) {
        os << "Partition{budget=" << p.budget << ", current_cost=" << p.current_cost << "}" << std::endl; 
...
public: 
    ...
    bool isReadyForNegotiation(std::string tensor_name, int32_t global_step) {
        ...
        return partitionSet.find(tensor_name) != partitionSet.end(); // is present
    }
...
private:
    ...
    void bin_pack() {
        ...
        std::cout << "When starting bin packing, the tensors are: " << std::endl;
        ...
        return t1->size > t2->size ||  (t1->size == t2->size && t1->name > t2->name);
        };
        std::sort(tensors.begin(), tensors.end(), grt); 

        if(tensors[0]->size > budget) {
            std::cout << "Infeasible to bin-pack the tensors with small budget. Provide higher fraction." << std::endl;
            throw "Infeasible to bin-pack the tensors with small budget. Provide higher fraction.";
            return;
        }
    ...
    void print_partition_info() {
        std::cout << "Initial total number of tensors: " << countGradients << std::endl;
        ...
        std::cout << "Total number of tensors in partitions: " << countTensorsFromParts << std::endl;
