...
REGISTER_KUNGFU_OP(SpotnikRequestVariable)
    .Attr("T: {int32, int64, float16, float32, float64}")
    .Attr("tensor_name: string")
    .Attr("shape: shape")
    .Attr("use_version: bool")
...
class SpotnikRequestVariable : public AsyncOpKernel
{
    std::string tensor_name_;
    TensorShapeProto shape_;
    bool use_version_;
  ...
  public:
    explicit SpotnikRequestVariable(OpKernelConstruction *context)
        : AsyncOpKernel(context)
    {
        OP_REQUIRES_OK(context, context->GetAttr("tensor_name", &tensor_name_));
        OP_REQUIRES(context, tensor_name_.size() >= 0,
...
REGISTER_KUNGFU_KERNEL_BUILDER(SpotnikRequestVariable, DEVICE_CPU);
