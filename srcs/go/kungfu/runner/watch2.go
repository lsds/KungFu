...
func (w *watcher2) create(id plan.PeerID, s Stage) {
	...
	go func(g *sync.WaitGroup) {
		runProc2(w.ctx, w.cancel, proc, s.Version, w.job.LogDir)
		g.Done()
		w.gpuPool.Put(gpuID)
		w.stopped <- id
	}(w.gs[id])
...
func (w *watcher2) delete(id plan.PeerID) {
	w.gs[id].Wait()
	delete(w.gs, id)
}

func (w *watcher2) update(s Stage) {
...
func (w *watcher2) watchRun(globalCtx context.Context) {
	for {
		select {
		case s := <-w.ch:
			w.update(s)
...
func WatchRun2(ctx context.Context, self plan.PeerID, runners plan.PeerList, ch chan Stage, j job.Job, keep bool, debugPort int) {
...
func runProc2(ctx context.Context, cancel context.CancelFunc, p proc.Proc, version int, logDir string) {
	r := &local.Runner{
		Name:          p.Name,
		LogDir:        logDir,
