...
func (c cluster) Start(ctx context.Context, all *sync.WaitGroup, name string, cmd string, args ...string) *node {
	ip := c.pool.get()
	wg := &sync.WaitGroup{}
	all.Add(1)
	wg.Add(1)
	go func() {
		c.Run(ctx, name, ip, cmd, args...)
		wg.Done()
		all.Done()
...
func (c cluster) Run(ctx context.Context, name, ip string, cmd string, args ...string) {
	dockerArgs := []string{
		`run`, `--rm`,
		`--name`, name,
		`--network`, c.vnet,
	...
	run(`docker`, append(dockerArgs, args...)...)
}
